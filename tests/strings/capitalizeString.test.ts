...
describe("capitalizeString with options", () => {
  ...
  it("should trim spaces at start and end if trim option is true", () => {
    ...
    expect(capitalizeString("   openai    ", { trim: true })).toBe("Openai");
  });
  ...
  it("should collapse multiple spaces into one if collapseSpaces is true", () => {
    ...
    expect(
      capitalizeString("this    is   a test", { collapseSpaces: true })
    ).toBe("This Is A Test");
  });
  ...
  it("should do both trim and collapseSpaces if both options are true", () => {
    ...
    expect(
      capitalizeString("   multiple    spaces   here  ", {
        trim: true,
        collapseSpaces: true,
      })
  ...
  it("should handle empty string", () => {
    expect(capitalizeString("", { trim: true, collapseSpaces: true })).toBe("");
    expect(capitalizeString("    ", { trim: true, collapseSpaces: true })).toBe(
      ""
    );
  });
  ...
  it("should handle null or undefined input", () => {
    expect(capitalizeString(null, { trim: true })).toBe("");
    expect(capitalizeString(undefined, { collapseSpaces: true })).toBe("");
  });

  it("should handle single word input", () => {
    expect(capitalizeString("hello", { trim: true })).toBe("Hello");
    expect(capitalizeString("HELLO", { collapseSpaces: true })).toBe("Hello");
  });

  it("should handle strings with numbers and special chars", () => {
    expect(capitalizeString("123 abc", { collapseSpaces: true })).toBe(
      "123 Abc"
    );
    expect(capitalizeString("!hello @world", { trim: true })).toBe(
      "!hello @world"
...
describe("capitalizeString with options", () => {
  it("should capitalize normally without options on long sentences", () => {
    expect(capitalizeString("this is a long sentence with many words")).toBe(
      "This Is A Long Sentence With Many Words"
    );

    expect(
      capitalizeString(" another   example    with    excessive    spaces ")
    ).toBe(" Another   Example    With    Excessive    Spaces ");
  });

  it("should handle multiple sentences separated by periods or exclamation marks", () => {
    expect(capitalizeString("hello world. this is a test! do it now.")).toBe(
      "Hello World. This Is A Test! Do It Now."
    );

    expect(capitalizeString(" wait... is this working? yes, it is ")).toBe(
      " Wait... Is This Working? Yes, It Is "
    );
  });

  it("should process text with special characters, emojis and mixed punctuation", () => {
    expect(capitalizeString("hello 😊 world! how's everything?")).toBe(
      "Hello 😊 World! How's Everything?"
    );

    expect(capitalizeString("@user #hashtag $dollar ^caret &and *star")).toBe(
  ...
  it("should correctly process three or more sentences combined", () => {
    ...
    expect(capitalizeString(input)).toBe(
      "This Is Sentence One. Here Comes Sentence Two! Finally, Sentence Three?"
    );
  });
  ...
  it("should work properly when both trim and collapseSpaces are enabled on complex input", () => {
    ...
    expect(capitalizeString(input, { trim: true, collapseSpaces: true })).toBe(
      "This Is A Long Line With Weird Spacing. Next Line? Done."
    );
  });

  it("should handle input with numbers interleaved with text", () => {
    expect(capitalizeString("123 testing 456 number 789 sequences")).toBe(
      "123 Testing 456 Number 789 Sequences"
    );

    expect(
      capitalizeString("  42 is    the answer   ", {
        trim: true,
        collapseSpaces: true,
      })
  ...
  it("should handle edge case of only special characters", () => {
    expect(capitalizeString(" @#$%^&*()_+ ")).toBe(" @#$%^&*()_+ ");
    expect(
      capitalizeString(" @#$%^&*()_+ ", { trim: true, collapseSpaces: true })
    ).toBe("@#$%^&*()_+");
  });
});
