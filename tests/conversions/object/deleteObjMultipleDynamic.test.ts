...
import { deleteObjMultipleDynamic } from "@/index";

describe("deleteObjMultipleDynamic - extreme variations and edge cases", () => {
  ...
  it("handles shallow delete where key is root level property", () => {
    ...
    const result = deleteObjMultipleDynamic(obj, [{ key: "b" }]);
  ...
  it("handles deepClone=false by mutating original object", () => {
    ...
    const result = deleteObjMultipleDynamic(
      obj,
      [{ key: "a.b.d", deep: true }],
      false
    );
  ...
  it("safe with completely empty object and some keys", () => {
    // @ts-expect-error
    const result = deleteObjMultipleDynamic({}, [{ key: "whatever" }]);
    expect(result).toEqual({});
  });
