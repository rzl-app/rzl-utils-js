import { deleteObjMultipleDynamicDeprecated } from "@/conversions/object/deprecated";
import { describe, expect, it } from "vitest";

describe("deleteObjMultipleDynamicDeprecated - exhaustive tests", () => {
  it("handles empty object", () => {
    const obj = {};
    const result = deleteObjMultipleDynamicDeprecated(
      obj,
  ...
  it("does nothing on non-object types", () => {
    ...
    expect(deleteObjMultipleDynamicDeprecated(null, [{ key: "a" }])).toEqual(
      {}
    );
    expect(
      deleteObjMultipleDynamicDeprecated(123 as any, [{ key: "a" }])
    ).toEqual({});
    expect(
      deleteObjMultipleDynamicDeprecated("abc" as any, [{ key: "a" }])
    ).toEqual({});
    expect(
      deleteObjMultipleDynamicDeprecated(true as any, [{ key: "a" }])
    ).toEqual({});
    expect(
      deleteObjMultipleDynamicDeprecated(undefined as any, [{ key: "a" }])
    ).toEqual({});
  });
